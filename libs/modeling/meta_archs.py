...
class IAL(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(DWConv_ks, int):
            self.DWConv_ks = [DWConv_ks] * len(self.fpn_strides)
        else:
            assert len(DWConv_ks) == len(self.fpn_strides)
            self.DWConv_ks = DWConv_ks
        max_div_factor = 1
        self.feat_win_size_1 = [1] + [feat_win_size] * backbone_arch[-1]
        for l, (s, w) in enumerate(zip(self.fpn_strides, self.feat_win_size_1)):
        ...
        # backbone network: conv + ConvFormer
        assert backbone_type in ['conv', 'ConvFormer']
        if backbone_type == 'ConvFormer':
            self.backbone = make_backbone(
                'ConvFormer',
                **{
                    'n_in': input_dim,
                    'n_embd': embd_dim,
                    'block_mlp_dim': block_mlp_dim,
                    ...
                    'DWConv_ks': self.DWConv_ks,
                    'use_abs_pe': use_abs_pe,
                    'MaxPool_ks': MaxPool_ks,
                    'init_conv_vars': init_conv_vars,
                    'LFE_lambda': LFE_lambda,
                    'use_mask': use_mask,
                    'two_short_cut': two_short_cut,
                }
            )
    ...
    def label_points(self, points, gt_segments, gt_labels):
        ...
        for gt_segment, gt_label in zip(gt_segments, gt_labels):
            cls_targets, reg_targets = self.label_points_single_video(points,
    ...
    @torch.no_grad()
    def label_points_single_video(self, points, concat_points, gt_segment, gt_label):
        ...
        if self.train_center_sample == 'radius':
            ...
            # F T x N
            inside_gt_seg_mask = center_seg.min(-1)[0] > 0 # (4536, 4)表示某个点是否在某个动作片段内
        else:
            # inside an gt action
        ...
        if self.multi_label:
            ...
            # corner case: multiple actions with very similar durations (e.g., THUMOS14)   (FT,N)
    ...
    def inference_single_video(
        ...
        for cls_i, offsets_i, pts_i, mask_i, sb_cls_i, eb_cls_i in zip(
            ...
            if self.use_trident_head:
                ...
                if self.multi_label:
                    pred_start_neighbours = pred_start_neighbours.transpose(0, 1)  # T, cls_num, num_bins
                    pred_end_neighbours = pred_end_neighbours.transpose(0, 1)  # T, cls_num, num_bins
            else:
