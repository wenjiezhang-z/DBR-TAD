...
class MultiTHUMOS14Dataset(Dataset):
    def __init__(
            ...
            i3d_feat_folder,  # folder for features
            videomaev2_feat_folder,
            ...
            force_upsampling,
            encoder_type,  # feat_type
            additional_feat_folder=None,
            rgb_only=False,
    ):
        # file path
        if encoder_type=='VideoMAEv2':
            assert os.path.exists(videomaev2_feat_folder) and os.path.exists(json_file)
        else:
            assert os.path.exists(i3d_feat_folder) and os.path.exists(json_file)
        assert isinstance(split, tuple) or isinstance(split, list)
        assert crop_ratio == None or len(crop_ratio) == 2
        self.videomaev2_feat_folder = videomaev2_feat_folder
        self.i3d_feat_folder = i3d_feat_folder
    ...
    def _load_json_db(self, json_file):
        ...
        for key, value in json_db.items():
            ...
            feat_file = os.path.join(self.i3d_feat_folder,
    ...
    def __getitem__(self, idx):
        ...
        # load features
        filename = os.path.join(self.i3d_feat_folder,
                                self.file_prefix + video_item['id'] + self.file_ext)
        feats_i3d = np.load(filename).astype(np.float32)
            
        feats_mae=np.load(os.path.join(self.videomaev2_feat_folder,video_item['id']+'.npy')).astype(np.float32)
        ...
        if self.encoder_type == 'i3d':
            ...
            if self.rgb_only:
                feats = feats[:, :1024]
        elif self.encoder_type == 'VideoMAEv2':
            feats = feats_mae
